...
test("recharge charges GST, credits only the base once, and renders the invoice breakdown", async (t) => {
  ...
  t.mock.method(globalThis, "fetch", async (url: string, init: RequestInit) => {
    ...
    assert.equal(body.amount, 1180);
    assert.equal(body.notes.credits, "10.00");
    assert.equal(body.notes.taxMinor, "180");
  ...
  await createRazorpayTopUp({ organization: { id: "org_test" }, body: { amountCredits: 10, taxMinor: 0, amount: 1000 } } as never, response as never);
  assert.equal(checkout?.amount, 1180);
  assert.equal(checkout?.credits, 10);
  assert.equal(checkout?.subtotalMinor, 1000);
  assert.equal(checkout?.taxMinor, 180);
  assert.equal(checkout?.taxRateBps, 1800);

  const payment = { id: "pay_gst", order_id: "order_gst", amount: 1180, currency: "USD", status: "captured" as const };
  ...
  assert.equal(savedInvoice?.amountPaid, 1180);
  assert.equal(savedInvoice?.subtotalMinor, 1000);
  assert.equal(savedInvoice?.taxMinor, 180);
  ...
  assert.match(html, /\$10\.00<\/strong>/);
  assert.match(html, /\$1\.80<\/strong>/);
  assert.match(html, /Total paid: \$11\.80/);
});
