...
const voiceAgentSchema = new Schema(
  {
    ...
    realtimeProvider: {
      type: String,
      enum: ["openai", "gemini", "inworld"],
      default: "gemini",
    },
    realtimeModel: { type: String, trim: true, default: "gemini-3.1-flash-live-preview" },
    llmProvider: {
      type: String,
      enum: ["openai", "gemini", "sarvam", "inworld"],
      default: "openai",
    },
    llmModel: { type: String, trim: true, default: "gpt-4.1-mini" },
    sttProvider: {
      type: String,
      enum: ["openai", "sarvam", "elevenlabs", "deepgram", "inworld", "cartesia"],
      default: "openai",
    },
    sttModel: { type: String, trim: true, default: "gpt-4o-mini-transcribe" },
    ttsProvider: {
      type: String,
      enum: ["openai", "gemini", "sarvam", "elevenlabs", "inworld", "cartesia"],
      default: "openai",
    },
    ...
    latencyMetrics: {
      ...
      recentSamplesMs: { type: [Number], default: [], select: false },
      minMs: { type: Number, min: 0 },
      p50Ms: { type: Number, min: 0 },
      p90Ms: { type: Number, min: 0 },
      p95Ms: { type: Number, min: 0 },
      p99Ms: { type: Number, min: 0 },
      maxMs: { type: Number, min: 0 },
